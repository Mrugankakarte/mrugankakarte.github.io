...
pubDate: 2026-03-27T10:00:31+05:30
image:
...
import graph1 from '../../assets/images/fashion/Graph1.png';
import Img1 from '../../assets/images/fashion/samecat_1.png';
...
<BlogImage src={graph1} alt="Sample graph from training batch" caption="Subgraph of graph from training batch"/>

Let's look at an example from our project and build a graph using the data from Polyvore Dataset.

We load a pre-built `HeteroData` object. We will focus on how to build a graph in upcoming posts.
```python
import torch
...
- The concept of heterogeneous graphs and their multi-entity nature.
- The use of the `HeteroData` object in PyG to manage complex graph structures.
- How the `to_hetero` function simplifies the conversion of standard GNNs into heterogeneous variants.
- The high-level architecture of a model designed for link prediction in an e-commerce context.

### What's Next?
Now that we understand the architecture of a Heterogeneous GNN, the next question is: how do we actually build the graph from our raw data? In the <a href="/blog/graph-neural-networks-for-outfit-completion---part-2"> next post</a>, *Building Your Heterogeneous Graph*, we will look into how to transform tabular DataFrames into a `HeteroData` object using the PyTorch Geometric library.

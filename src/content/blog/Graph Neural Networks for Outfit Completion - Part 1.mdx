...
### What's Next?
Now that we understand the architecture of a Heterogeneous GNN, the next question is: how do we actually build the graph from our raw data? In the next post, *Building Your First Heterogeneous Graph*, we will look into how to transform tabular DataFrames into a `HeteroData` object using the PyTorch Geometric library.
